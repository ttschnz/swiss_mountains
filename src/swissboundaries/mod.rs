...
pub async fn get_region(point: (u32,u32), region_type: RegionType, conn: &Connection)->Result<Vec<String>>{
    let rows = conn.call(move |conn|{
        let mut stmt  = conn.prepare(
            ...
            ", region_type.get_table_name()).as_str(),
        )?;
...
mod test{
    ...
    async fn test_query(){
        ...
        let regions = get_region((2616381, 1166112), RegionType::HOHEITSGEBIET, &connection).await.unwrap();
        assert_eq!(regions, vec!["Reichenbach im Kandertal"], "regions of type 'HOHEITSGEBIET' don't match ");
        let regions = get_region((2616381, 1166112), RegionType::BEZIRKSGEBIET, &connection).await.unwrap();
        assert_eq!(regions, vec!["Frutigen-Niedersimmental"], "regions of type 'BEZIRKSGEBIET' don't match ");
        let regions = get_region((2616381, 1166112), RegionType::KANTONSGEBIET, &connection).await.unwrap();
        assert_eq!(regions, vec!["Bern"], "regions of type 'KANTONSGEBIET' don't match ");
        let regions = get_region((2616381, 1166112), RegionType::LANDESGEBIET, &connection).await.unwrap();
        assert_eq!(regions, vec!["Schweiz"], "regions of type 'LANDESGEBIET' don't match ");
    }
}